...
x='';
 $.ajax({ url: "/gstaffmaintainancedetail",
      data: x,
 ...
 for(i=0;i<t.length;i++)
 {
  ...
  document.getElementById("inserthere").innerHTML='<div>The available slots are: '+s+'</div><br><div>Enter the slot to be closed for maintainance</div><input type="text" id="inputofuser"><br><input type="button" value="submit" onclick="check()">';
...
function check(){
  ...
  if(window.j.includes(p))
  {
    ...
    document.getElementById("inserthere2").innerHTML='<div>'+'Invalid Entry'+'</div>';
}
...
console.log(sdata);
$.ajax({ url: "/gstaffmaintainancedetail2",
      data: sdata,
      success: function(result){
        console.log('chala');
        console.log(result);
